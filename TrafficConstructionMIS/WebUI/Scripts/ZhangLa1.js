function ZhangLaTable(id) {
    ...
    this.initTable = function () {
        ...
        $("#table_ZhangLa").jqGrid({
            ...
            rowList: [10, 20, 30],
            rownumbers: true,
            colNames: ['Id','颜色', '梁号', '孔号', '张拉次数', '梁型', '张拉类型', '控制应力（KN）', '理论伸长量（mm）', '实际伸长量（mm）', '相对误差率（%）', '保压时间（s）', '张拉日期'],
            colModel: [
                {
                    name: 'Id',
                    index: 'Id',
            ...
            gridComplete: function () {
                var ids = $("#table_ZhangLa").getDataIDs();
                for (var i = 0; i < ids.length; i++) {
                    var rowData = $("#table_ZhangLa").getRowData(ids[i]);
                    if (rowData.Color != "white") {//如果审核不通过，则背景色置于红色
